...
>>>>>>> af2809605aa7a304d96d83b7ba7d51f6acbbe402
...
##EVERYTHING BELOW IS JUST EXTRA CODE and NOTES
##Analysis One: Death penalty status as explanatory variable 
...
##A bivariate plot with your explanatory variable (Death Penalty Status, DP) on the x-axis, and your outcome variable (Homicide rate, MNNM_pop) on the y-axis
DP_plot<- alldata|> 
  mutate(DPYN = if_else(DP == 1, "Death Penalty", "No Death Penalty")) |>
  ggplot(mapping = aes(x=DPYN, y=MNNM_pop, group =DPYN)) +
  geom_point() +
...
##Do permutation test to see if this could be due to random chance 
nulldist<- alldata |>
mutate(DPyn = if_else(DP == 1, "DPy", "DPn")) |>
specify(MNNM_pop~ DPyn) |>
hypothesize(null = "independence") |>
...
## A table containing regression results from your multivariate model.
DP_hom_reg_basic<- lm(MNNM_pop~DP, data = alldata)
tidy(DP_hom_reg_basic) |>
knitr::kable(digits = 3)
DP_hom_reg<- lm(MNNM_pop ~ DP + violent_pop, data = alldata)
...
```{r}
##Analysis two: Executions or sentences as explanatory variable 

##Bivariate plot with explanatory variable (Exs and sent) on x axis and outcome variable (MNNM_pop) on Y axis
alldata |> 
  filter(sent>0)
executions_plot<- alldata |> 
  filter(DP==1)|>
   mutate(exsyn = if_else(exs>0, "Execution", "No Execution")) |>
  ggplot(mapping = aes(x=exsyn, y=MNNM_pop, group =exsyn)) +
  geom_point() +
    labs(title= "Homicide Rate by County vs Executions in County", x = "Executions in County", y = "Homicide rate per 100,000 Population")
executions_plot

sentences_plot<- alldata |>
  filter(DP==1)|>
   mutate(sentyn = if_else(sent>0, "Sentences", "No Sentences")) |>
  ggplot(mapping = aes(x=sentyn, y=MNNM_pop, group =sentyn)) +
  geom_point() +
    labs(title= "Homicide Rate by County vs Death Sentences in County", x = "Death Sentences in County", y = "Homicide rate per 100,000 Population")
sentences_plot

##A table containing regression results from multivariate model 
exssent_reg<- lm(MNNM_pop ~ exs + sent, data = alldata)
tidy(exssent_reg) |>
knitr::kable(digits = 3)
```

```{r}
##Change in homicide rate pre and post abolition
##This applies to states: Delaware 2016, Illinois 2011, Washington 2018, New Hampshire 2019, Maryland 2013
##Illinois
IL_change<- alldata|> 
  mutate(pre2011 = if_else(Year < 2011, "Before 2011", "After 2011"), Illinois = if_else(State == 'Illinois', "Illinois", "Other"))|>
  group_by(Illinois, pre2011)|>
