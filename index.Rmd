...
##This table shows us that maryland's homicide count was higher (at roughly 5.6 homicides on average in each county) before eliminating the death penalty than after eliminating it (roughly 3.64 homicides per county). While this is not causal, this is certainly in line with other findings about the effectivness of the death penalty. 
...
alldata<- alldata|>
  mutate(violent_pop = (Violent_crime/estimate))
alldata <- alldata|>
  mutate(MNNM_pop = MNNM/estimate)

```

```{r}
##Two ideas to test: County vs County or County vs Self.
...
##County v self-- Refine data to only states that got rid of the DP within the data set 
## Test 1: homicide rate controlled by VC in year before vs after abolition for all counties
## Test 2: then homicide rate controlled by VC in year before vs after abolition for counties which did actually execute someone in the last year
## Test 3: Then homicide rate controlled by VC in year before vs after abolition for counties which sentenced someone in last year 
